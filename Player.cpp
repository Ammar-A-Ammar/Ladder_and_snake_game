...
void Player::SetjustRolledDiceNum(int x)
{
	justRolledDiceNum = x;
}
...
void Player::Move(Grid* pGrid, int diceNumber)
{
	...
	Player p1(pCell, playerNum);
	Player* p11 = &p1;
	pGrid->UpdatePlayerCell(p11, pos);

	// 6- Apply() the game object of the reached cell (if any)
	GameObject *go= pCell->GetGameObject();
	go->Apply(pGrid, p11);
	// 7- Check if the player reached the end cell of the whole game, and if yes, Set end game with true: pGrid->SetEndGame(true)
	CellPosition finalpos = pCell->GetCellPosition();
	if (finalpos.GetCellNum()==99)
	{
