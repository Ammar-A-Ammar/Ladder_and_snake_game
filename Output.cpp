...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICEVALUE] = "images\\Menu_AddDiceV.jpg";
	MenuItemImages[ITM_OFFER_MORTGAGE] = "images\\Menu_Offer.jpg";
	MenuItemImages[ITM_ACCEPT_MORTGAGE] = "images\\Menu_Accept.jpg";
	MenuItemImages[ITM_RETURN_MORTGAGE] = "images\\Menu_Return.jpg";
	MenuItemImages[ITM_SELLCITY] = "images\\Menu_Sellcity.jpg";
	MenuItemImages[ITM_NEWGAME] = "images\\Menu_Restart.jpg";
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	pWind->DrawLine(x34, y1, x34, y2);                                                       // the y coordinates is the same as the First Vertical Line
	...
	int ff = GetCellStartY(toCell);
	
	for (int i = ff+UI.CellHeight;i<=fromStartY;i)
	{
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	//int y[4] = {300,200,300,400}; 
	int y[4] = { y1 + 10,y1,y1 + 10,y1 + 20 };
...
void Output::DrawCoinSet(const CellPosition & cellPos) const
{
	...
	image c("images\\Menu_coinset.jpg", JPEG);
	pWind->DrawImage(c, cellStartX, cellStartY, UI.CellWidth, UI.CellHeight);
