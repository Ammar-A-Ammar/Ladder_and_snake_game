...
int Input::GetInteger(Output* pO) const
{
	...
	while (1)
	{
		pWind->WaitKeyPress(Key);
		if (Key == 27)	// ESCAPE key is pressed
			return stoi("");	// returns nothing as user has cancelled label
		if (Key == 13)	// ENTER key is pressed
			return stoi(Label);
		if ((Key == 8) && (Label.size() >= 1))	// BackSpace is pressed
			Label.resize(Label.size() - 1);
...
ActionType Input::GetUserAction() const
{
	...
	if (UI.InterfaceMode == MODE_DESIGN)
	{
		...
		if (y >= 0 && y < UI.ToolBarHeight)
		{
			...
			case ITM_COPY:return COPY_CARDCOIN;
			case ITM_CUT:return CUT_CARDCOIN;
			case ITM_PASTE:return PASTE_CARDCOIN;
			case ITM_DELETE: return DELETE_OBJ;
	...
	else
	{
		if (y >= 0 && y < UI.ToolBarHeight)
		{
			...
			case ITM_INPUT_DICEVALUE:return INPUT_DICEV;
			...
			case ITM_Exit:return Exit;
				///TODO: Add cases for the other items of game Mode
